...
const STORAGE_KEY = '@daehan_quest_land_v2';
...
const initialState = {
  ...
  quests: [
    {
      id: '1', title: '수학 숙제 완료하기',
      sub: '문제집 2페이지 풀기',
      type: 'study', difficulty: 'medium', xp: 25,
      completed: false, completedAt: null, isDaily: true,
    },
    {
      id: '2', title: '양치질 2번 하기',
      sub: '아침·저녁 2분씩 꼼꼼하게',
      type: 'health', difficulty: 'easy', xp: 10,
      completed: false, completedAt: null, isDaily: true,
    },
    {
      id: '3', title: '책 20분 읽기',
      sub: '좋아하는 책으로 20분!',
      type: 'reading', difficulty: 'medium', xp: 20,
      completed: true, completedAt: '2026-05-15T08:00:00.000Z', isDaily: true,
    },
    {
      id: '4', title: '내일 준비물 챙기기',
      sub: '미리 가방 싸두기',
      type: 'prep', difficulty: 'easy', xp: 15,
      completed: false, completedAt: null, isDaily: true,
    },
    {
      id: '5', title: '물 6잔 마시기',
      sub: '하루 동안 나눠 마시기',
      type: 'water', difficulty: 'easy', xp: 10,
      completed: false, completedAt: null, isDaily: true,
      progress: 0.4, special: false,
    },
    {
      id: '6', title: '30분 운동하기',
      sub: '달리기, 줄넘기 등 자유롭게',
      type: 'exercise', difficulty: 'hard', xp: 40,
      completed: false, completedAt: null, isDaily: false,
      special: true,
    },
  ],
  rewards: [
  ...
  badges: [
    { id: 'b1', title: '첫 퀘스트',   emoji: '⭐', kind: 'star',   earned: true,  condition: '첫 퀘스트 완료' },
    { id: 'b2', title: '연속 3일',    emoji: '🔥', kind: 'flame',  earned: true,  condition: '3일 연속 완료' },
    { id: 'b3', title: '연속 7일',    emoji: '💎', kind: 'bolt',   earned: false, condition: '7일 연속 완료' },
    { id: 'b4', title: '책벌레',      emoji: '📖', kind: 'book',   earned: true,  condition: '독서 10회 완료' },
...
function xpForLevel(level) {
  return XP_PER_LEVEL[Math.min(level, XP_PER_LEVEL.length - 1)] || level * 1200;
}

function calcLevel(totalXP) {
  let lv = 1;
  for (let i = 2; i <= 10; i++) {
    if (totalXP >= xpForLevel(i)) lv = i;
    else break;
  }
  return lv;
}
...
const TITLES = [
  '', '퀘스트 초보자', '용감한 탐험가', '퀘스트 전사',
  '영웅 견습생', '실력자', '퀘스트 마스터', '전설의 용사',
  '영웅', '슈퍼히어로', '전설',
];
...
function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'COMPLETE_QUEST': {
      ...
      const newStreak =
        last === today ? state.player.streak
        : last === yesterday.toDateString() ? state.player.streak + 1
        : 1;
      ...
      return {
        ...
        quests: state.quests.map((q) =>
          q.id === action.questId ? { ...q, completed: true, completedAt: new Date().toISOString() } : q
        ),
    ...
    case 'REDEEM_REWARD': {
      ...
      const updatedBadges = state.badges.map((b) =>
        b.id === 'b7' && !b.earned && state.rewardHistory.length === 0
          ? { ...b, earned: true } : b
      );
      return {
        ...state,
